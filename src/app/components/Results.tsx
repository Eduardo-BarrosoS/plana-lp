...
export function Results({ id }: { id?: string }) {
  return (
    <div
      id={id}
      className="container mx-auto px-4 md:px-8 lg:px-0 max-w-[1000px] overflow-hidden flex flex-col gap-10 md:gap-16 lg:gap-20 py-10 md:py-16 lg:py-20 items-start"
    >
      <div className="flex flex-col lg:flex-row  items-center gap-8 lg:gap-24">
        <div className="flex w-full lg:w-[45%] flex-col gap-3 text-center lg:text-left">
          <h2 className="font-bold text-2xl md:text-3xl lg:text-[42px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 w-full mx-auto justify-between items-center gap-8 md:gap-0">
        {results.map((result, index) => (
          <div
            ...
            key={index}
          >
            <h3 className="font-medium text-3xl md:text-3xl text-[#2C2C2C]">
              {result.title}
            </h3>
